...
c = "50"
train_file = '../data/caltech_train_'+ c +'images.txt'
...
with tf.name_scope("train"):
    ...
    gradients = list(zip(gradients, var_list)) #for python3 we can cant subscript zip object
    ...
    optimizer_1 = tf.train.GradientDescentOptimizer(learning_rate=0.001)
    train_op_1 = optimizer_1.apply_gradients(gradients[0:1])
    
    optimizer_2 = tf.train.GradientDescentOptimizer(learning_rate=learning_rate)
    train_op_2 = optimizer_2.apply_gradients(gradients[2:3])
...
with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
    ...
    for step in range(train_batches_per_epoch):
        ...
        train.append(np.mean(feat, axis=0))

    train_ar = np.array(train)
    np.save("array_train"+ c, train_ar)
